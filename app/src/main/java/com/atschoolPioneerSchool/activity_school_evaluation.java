...
import com.atschoolPioneerSchool.adapter.EvaluationsListAdapter;
 ...
 */
public class activity_school_evaluation extends AppCompatActivity {
    /**
    ...
    public EvaluationsListAdapter mAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isNetworkAvailable(getBaseContext())) {
            ...
            new activity_school_evaluation.GetDataTask(this).execute("");
        }
    }

    private class GetDataTask extends AsyncTask<String, Evaluation, String> {

        private Context mContext;
        private int cntall;
        private List<Evaluation> items = new ArrayList<>();
        private double Balance = 0;

        //http://schoolrootweb.controporal.com/API_Mobile.aspx?events=10&username=omar1&pass=omar1&AuthenticationKey=389206&Student_Id=2238&STUDY_YEARS_HDR_ID=309230
        String tag[] = {"events", "username", "pass", "AuthenticationKey", "Student_Id"};
        String value[] = {"10", username, pass, AuthenticationKey, Student_Id};
        ...
        public GetDataTask(Context context) {
            ...
            //  ((activity_school_Evaluations) mContext).setTitle(" كشف حساب " + "    " + " الرصيد " + Balance);
        }
                    ...
                    if (!json_code.equals("")) {
                        ...
                        if (jsonArray.length() > 0) {
                            ...
                            for (i = 0; i < cntall; i++) {
                                ...
                                try {
                                    ...
                                    if (Message_Id > 0) {
                                        ...
                                        Evaluation objEvaluation = new Evaluation(i + 1, 0, "", ""
                                                , json.getString("StudentId"), json.getString("FName"), json.getString("FNameA")
                                                , json.getString("subjectName"), json.getString("subjectNameA")
                                                , json.getString("DESCRIPTION"), json.getString("DESCRIPTIONA")
                                                , json.getString("MASTERY_EVALUATION_Name_Term1"), json.getString("MASTERY_EVALUATION_Name_Term1A")
                                                , json.getString("MASTERY_EVALUATION_Name_Term2"), json.getString("MASTERY_EVALUATION_Name_Term2A")
                                                , json.getString("MASTERY_EVALUATION_Name_Term3"), json.getString("MASTERY_EVALUATION_Name_Term3A")
                                                , false);

                                        items.add(objEvaluation);

                                        //set Balance in title
                                        if (i == 0) {
                                            Balance = json.getDouble("Balance");

                                            // publishProgress(objEvaluation);
                           ...
                           //add last row the balance
                            if (Message_Id == 0) {
                                Evaluation objEvaluation = new Evaluation(i + 1, 0, "", "", 0, 0,0, 0, Balance, "", "","", "", "", "", true);
        ...
        @Override
        protected void onProgressUpdate(Evaluation... values) {
        ...
        protected void onPostExecute(String msg) {
            ...
            mAdapter = new EvaluationsListAdapter(mContext, items);
